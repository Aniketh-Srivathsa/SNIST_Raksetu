...
export default function NotificationCenter({ user, userProfile, onClose }) {
        ...
        timeoutId = setTimeout(() => {
          const notificationsList = snapshot.docs.map(doc => {
            ...
            return {
              ...
              isDismissed: data.dismissedBy?.includes(user.uid) || false
            };
          });
  ...
  }, [notifications, readNotification, user]);

  // Dismiss notification for the current user
  ...
  const clearAll = useCallback(async () => {
    try {
      await Promise.all(
        notifications.map(notif => dismissNotification(notif.id))
      );
    } catch (error) {
