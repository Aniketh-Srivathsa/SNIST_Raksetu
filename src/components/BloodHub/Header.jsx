...
export default function Header({
  ...
  useEffect(() => {
    ...
    unsubscribe = listenForNotifications((newNotifications) => {
      ...
      const filteredNotifications = newNotifications.filter(notification => {
        if (isLoggedIn && auth.currentUser?.uid && notification.dismissedBy?.includes(auth.currentUser.uid)) {
          return false;
        }
  ...
  // Clear all notifications for the current user by dismissing shared documents
